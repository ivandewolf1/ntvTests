void inverseTest(GU_Detail *gdp, UT_Vector3D *origin, UT_Vector3D *lookat) {

    UT_Matrix3F m3HDK;
    UT_Vector3D upHDK(0.0,1.0,0.0);
    m3HDK.identity();
    m3HDK.lookat(*origin, *lookat, upHDK);
    UT_Matrix3F m3HDKinv = m3HDK;
    m3HDKinv.invert();
    m3HDK = m3HDK * m3HDKinv;
    ...
    ntv::nmat3<float> m3NTVinv = m3NTV.inverse();
    m3NTV = m3NTV * m3NTVinv;
    /*ntv::nmat3<float> m3NTVt = m3NTV;
    m3NTVt.transpose();
    ntv::nmat3<float> m3NTVinv = m3NTVt.inverse();
    m3NTVinv.transpose();
    m3NTV = m3NTV * m3NTVinv;*/

    std::cout << "\nm3HDK: \n" << m3HDKinv << std::endl;
    std::cout << "m3NTV: " << m3NTVinv << std::endl;
